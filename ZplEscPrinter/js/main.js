...
async function escpos(data){
    try{ data = atob(data).trim(); }catch(e){
        try{
            data = JSON.parse(
                '"' + data.replaceAll(/\\\\[u|U]0/g, '\\u0').replaceAll(/\\\\[x|X]/g, '\\x').replaceAll('"', '\\"')
...
function initEvents() {
    ...
    ipcRenderer.on('selected-dirs', function (event, response) {
